...
class FaissSemanticCache(_SemanticCacheBase):
    ...
    async def query_cache(
        ...
        res = await self.faiss_.asimilarity_search_with_score_by_vector(
            embedding=self.embed_messages(query["messages"], encoding_method=semantic_cache_encoding_method),
    ...
    async def add_cache(
        ...
        query = get_completion_create_params(**query)
        text = self.encode_messages(query["messages"], encoding_method=semantic_cache_encoding_method)
        ...
        self.faiss_.add_embeddings(
            text_embeddings=[
                (
                    text,
                    self.embed_messages(query["messages"], encoding_method=semantic_cache_encoding_method),
                )
            ],
            metadatas=[
                {
