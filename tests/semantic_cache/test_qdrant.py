...
async def test_exact(qdrant_cache, messages):
    ...
    res = await qdrant_cache._QdrantClientConfig.get_async_client().search(
        ...
        query_vector=qdrant_cache.embed_messages(
            [message], encoding_method="user_only"
        ),
        with_payload=True,
        with_vectors=True,
        limit=1,
...
async def test_close(qdrant_cache, messages):
    ...
    res_embedded = qdrant_cache.embed_messages(
        [res_message], encoding_method="user_only"
    )
